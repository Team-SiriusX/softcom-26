...
function docTextFromSnapshot(snapshot: DashboardSnapshot): string {
  ...
  const sections = [
    ...
    `  Revenue: Rs ${k.monthRevenue.toFixed(2)}`,
    `  Expenses: Rs ${k.monthExpenses.toFixed(2)}`,
    `  Net Income: Rs ${k.monthNetIncome.toFixed(2)} (${profitMargin}% margin)`,
    ``,
    `LIQUIDITY & CASH FLOW:`,
    `  Cash Balance: Rs ${k.cashLike.toFixed(2)}`,
    `  Working Capital: Rs ${k.workingCapital.toFixed(2)}`,
    `  Current Assets: Rs ${k.currentAssets.toFixed(2)}`,
    `  Current Liabilities: Rs ${k.currentLiabilities.toFixed(2)}`,
    `  Current Ratio: ${currentRatio}`,
    `  Quick Ratio: ${quickRatio}`,
    ``,
    `BURN RATE & RUNWAY (90-day avg):`,
    `  Monthly Burn Rate: Rs ${k.burnRate3mo.toFixed(2)}`,
